...
function configurarBuscador() {
    ...
    const idInput = document.getElementById("pokemonIdSearch");
    const typeFilter = document.getElementById("pokemonTypeFilter");

    if (!searchInput || !idInput || !typeFilter) return;

    // Función unificada que filtra por los 3 criterios
    const aplicarFiltros = () => {
        const nombreBusqueda = searchInput.value.toLowerCase();
        const idBusqueda = idInput.value;
        const tipoBusqueda = typeFilter.value;

        const filtrados = todosLosPokemon.filter(p => {
            // Condición de nombre
            const coincideNombre = p.name.includes(nombreBusqueda);
            
            // Condición de ID (si el campo está vacío, no filtra por ID)
            const coincideID = idBusqueda === "" || p.id.toString() === idBusqueda;
            
            // Condición de Tipo
            const coincideTipo = tipoBusqueda === "all" || 
                                p.types.some(t => t.type.name === tipoBusqueda);
    ...
    // Escuchar eventos en los tres elementos
